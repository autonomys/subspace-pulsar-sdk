...
impl<PersistentProviderStorage> FarmerProviderStorage<PersistentProviderStorage> {
    pub(crate) fn new(
        ...
        Self { local_peer_id, readers_and_pieces, persistent_provider_storage }
    }
}
...
{
    type ProvidedIter<'a> = std::iter::Chain<std::vec::IntoIter<Cow<'a, ProviderRecord>>, PersistentProviderStorage::ProvidedIter<'a>>
    where Self: 'a;
    ...
    fn remove_provider(&mut self, key: &Key, peer_id: &PeerId) {
        self.persistent_provider_storage.remove_provider(key, peer_id)
    }
}
